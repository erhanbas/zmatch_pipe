...
%% Check tile 2 descriptor
tile_2_fp = dir(fullfile(tile2, '*.mat'));
tile_2_fp = fullfile(tile_2_fp.folder, tile_2_fp.name);
tile_2_desc = load(tile_2_fp);
%% Check if the skeleton is from the image
% Load image
xy_match_1 = paireddescriptor{1};
xy_match_2 = paireddescriptor{2};
xy_match_3 = paireddescriptor{3};

xy_match_1{test_grid_idx}.ony
xy_match_2{test_grid_idx}.ony
%%
sub_1 = xy_match_1{test_grid_idx}.ony.X;
sub_2 = xy_match_1{test_grid_idx}.ony.Y;
figure;
scatter3(sub_1(:,2), sub_1(:,1), sub_1(:,3))
hold on 
scatter3(sub_2(:,2), sub_2(:,1), sub_2(:,3))